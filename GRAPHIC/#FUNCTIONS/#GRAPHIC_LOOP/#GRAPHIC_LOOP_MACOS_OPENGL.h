	...
	GRAPHIC_LOOP(struct GRAPHIC *GRAPHIC)
{
	...
	while (!!GRAPHIC->WINDOW_MODULE)
	{
       ...
       // Clear the color buffer
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, GRAPHIC->WIDTH, GRAPHIC->HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, GRAPHIC->BUFFER);

        // Use the shader program
        glUseProgram(GRAPHIC->PROGRAM);
        ...
        // Swap buffers
        CGLFlushDrawable(GRAPHIC->CONTEXT);
